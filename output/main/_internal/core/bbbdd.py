...
class RepositorioSnippets:
    def __init__(self):
        ...
        # Asegura que la base de datos se guarde en un lugar seguro
    ...
    def _inicializar_tablas(self):
        with sqlite3.connect(self.db_path) as conexion:
            ...
            # Activar claves foráneas para que al borrar una carpeta, se borren sus notas
            ...
            # Tabla de configuración (para guardar idiomas, rutas locales, etc.)
    ...
    # --- MÉTODOS DE BÓVEDA (CARPETAS Y NOTAS) ---
    ...
    def marcar_como_sincronizado(self, id):
        with sqlite3.connect(self.db_path) as c: 
            c.execute("UPDATE snippets SET sincronizado_cloud = 1 WHERE id = ?", (id,))
            c.commit()

    # --- NUEVA FUNCIÓN PARA GUARDAR EDICIONES COMPLETAS ---
